...
contract WigoNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
     ...
     * @param _WFTMAddress: WFTM address
    ...
    constructor(
        ...
        require(
            _WFTMAddress != address(0),
            "Operations: WFTM address cannot be zero"
        );
        require(
    ...
    /**
     * @notice Buy token with FTM by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     */
    function buyTokenUsingFTM(address _collection, uint256 _tokenId)
        external
        payable
        nonReentrant
    {
    ...
    /**
     * @notice Buy token with WFTM by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingWFTM(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(WFTM).safeTransferFrom(
            address(msg.sender),
            address(this),
            _price
        );
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(WFTM).safeTransfer(address(msg.sender), revenueToClaim);
     ...
     * @param _withFTM: whether the token is bought with FTM (true) or WFTM (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
        ...
        // Calculate the net price (collected by seller), trading fee (collected by treasury to burn), creator fee (collected by creator), referrer fee (collected by buyer and seller referrers)
        (
            uint256 netPrice,
            uint256 tradingFee,
            uint256 creatorFee
        ...
        // Transfer WFTM to seller
        IERC20(WFTM).safeTransfer(askOrder.seller, netPrice);

        // Transfer WFTM to referrals
    ...
    /**
     * @notice pay referrers fees
     * @param _seller: seller address
     * @param _buyer: buyer address
     * @param _askPrice: listed price
     */
    function _payReferrersFees(
        address _seller,
        address _buyer,
        uint256 _askPrice
    ) internal returns (uint256 count) {
        if (galaxy.hasRegistered(_seller)) {
            (address sellerRef, bool sellerRefIsActive, ) = galaxy
                .getReferralData(_seller);
        ...
        if (galaxy.hasRegistered(_buyer)) {
            (address buyerRef, bool buyerRefIsActive, ) = galaxy
                .getReferralData(_buyer);
